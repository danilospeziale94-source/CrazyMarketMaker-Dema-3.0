import os
import telebot
from dotenv import load_dotenv
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton

load_dotenv()
TOKEN = os.getenv("TELEGRAM_BOT_TOKEN")

bot = telebot.TeleBot(TOKEN)

def main_menu():
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(InlineKeyboardButton("🟢 1. Affiliazione PU Prime", callback_data="affiliazione"))
    markup.add(InlineKeyboardButton("💰 2. Effettua Pagamento 299€", callback_data="pagamento"))
    markup.add(InlineKeyboardButton("💬 3. Contattami in Privato", callback_data="contact"))
    markup.add(InlineKeyboardButton("📋 Prezzi Extra", callback_data="prezzi"))
    markup.add(InlineKeyboardButton("⚠️ Disclaimer", callback_data="disclaimer"))
    return markup

@bot.message_handler(commands=['start'])
def start(message):
    text = """
<b>👋 Benvenuto nel Bot Ufficiale DEMA 3.0</b>

Per accedere al sistema completo devi seguire questi passi in ordine:

1. Affiliazione a PU Prime
2. Pagamento del pacchetto
3. Contatto privato per attivazione

Scegli il primo passo:
"""
    bot.send_message(message.chat.id, text, parse_mode='HTML', reply_markup=main_menu())

@bot.callback_query_handler(func=lambda call: True)
def callback_handler(call):
    if call.data == "affiliazione":
        text = """
<b>🟢 1. AFFILIAZIONE PU PRIME</b>

1. Registrati cliccando qui:
👉 <a href="https://puvip.co/la-partners/B8gYmnJT">REGISTRATI SU PU PRIME</a>

2. Inserisci il codice: <b>B8gYmnJT</b>

3. Completa la verifica dell'identità.

<b>Dopo la registrazione invia questa email:</b>

A: info@puprime.com

Oggetto: Request to be moved under IB

Testo:
Hello, I want to be moved under IB CMM50

Una volta completata l'affiliazione, procedi al pagamento.
"""

    elif call.data == "pagamento":
        text = """
<b>💰 2. EFFETTUA IL PAGAMENTO</b>

Importo: <b>299€</b> (pacchetto DEMA 3.0 completo)

Bonifico a:
IBAN: <code>IT04M0306902506100000005672</code>
Intestato a: De martino Gennaro

Dopo aver effettuato il bonifico, vai al passaggio 3 (Contattami in Privato).
"""

    elif call.data == "contact":
        text = """
<b>💬 3. CONTATTAMI IN PRIVATO</b>

Hai completato:
- Affiliazione PU Prime
- Bonifico di 299€

Scrivi in privato a:
👉 @crazymarketmakerEA
o
👉 @Demafranzfx

Ti daremo accesso completo al sistema (indicatori, EA, Discord, ecc.).
"""

    elif call.data == "prezzi":
        text = """
<b>📋 PREZZI EXTRA</b>

• CMM50 → Accesso Discord
• CMM300 → Corso Full

Per info su questi pacchetti scrivi in privato.
"""

    elif call.data == "disclaimer":
        text = """
<b>⚠️ DISCLAIMER IMPORTANTE</b>

Il trading comporta un alto rischio di perdita del capitale.
I risultati passati non sono garanzia di risultati futuri.
Tutti gli strumenti hanno scopo educativo e informativo.
L'utente è l'unico responsabile delle proprie operazioni.
L'amministratore del bot non è responsabile per eventuali perdite.
"""

    bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=main_menu())

print("🚀 Bot DEMA 3.0 avviato con successo!")
bot.infinity_polling()